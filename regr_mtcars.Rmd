...
```{r}
index_test <- caret::createDataPartition(df, 
                           p = 0.75, 
                           list = FALSE)
...
```{r}
preproc_alg <- caret::preProcess(df_train, method = c("BoxCox", "center", "scale", "pca"))
df_test_prpr <- stats::predict(preproc_alg, df_train)
df_test_prpr
```
...
panelRange <- extendrange(df_test_prpr[, 1:3])
splom(as.data.frame(df_test_prpr[, 1:3]),
